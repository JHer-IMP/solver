...
def parse_order_data(dm: RealDistanceMatrix) -> List[Order]:  # 这个函数要修改
    """
    从附件2 Excel 文件中读取并解析订单数据。
    返回按（运输交接单号，收货地址）聚合的订单列表。
    """
    print("  读取附件2：排货信息表.xlsx ...")
    # 读取所有sheet
    xls = pd.ExcelFile("附件2 排货信息表.xlsx")
    all_data = []
    
    for sheet_name in xls.sheet_names:
        # 每个sheet的结构：第1-2行是标题/描述，第3行是列名，数据从第4行开始
        df = pd.read_excel(xls, sheet_name=sheet_name, header=2)
        # 重命名列为有意义的英文名（便于处理）
        df.columns = [
            "month", "fill_date", "doc_number", "temp_id", "trans_receipt_no",
            "boxes", "cases", "pallet_config", "pallets", "receiver_addr",
            "transit_days", "est_pickup_date", "est_arrival_date", "vehicle_model",
            "transport_mode", "vehicle_count", "origin_weather", "dest_weather",
            "actual_pickup_date", "actual_arrival_date", "sign_status", "doc_complete", "remark"
        ]
        # 过滤掉运输交接单号为空的行
        df = df.dropna(subset=['trans_receipt_no'])
        df['trans_receipt_no'] = df['trans_receipt_no'].astype(str).str.strip()
        df = df[df['trans_receipt_no'] != '']
        
        # 选取需要的列
        df = df[['trans_receipt_no', 'pallets', 'receiver_addr', 'transit_days', 'est_arrival_date']]
        all_data.append(df)
    ...
    # 类型转换
    full_df['pallets'] = pd.to_numeric(full_df['pallets'], errors='coerce')
    full_df['transit_days'] = pd.to_numeric(full_df['transit_days'], errors='coerce')
    # 预计到货日期转为datetime
    full_df['est_arrival_date'] = pd.to_datetime(full_df['est_arrival_date'], errors='coerce')
    # 剔除缺失关键数据的行
    full_df = full_df.dropna(subset=['pallets', 'transit_days', 'est_arrival_date', 'receiver_addr'])
    full_df['receiver_addr'] = full_df['receiver_addr'].astype(str).str.strip()
    
    # 按（运输交接单号, 收货地址）分组聚合
    grouped = full_df.groupby(['trans_receipt_no', 'receiver_addr'], dropna=False).agg(
        total_pallets=('pallets', 'sum'),
        transit_days=('transit_days', 'first'),          # 同组内应该相同，取第一个
        est_arrival_date=('est_arrival_date', 'first')
    ).reset_index()
    
    # 计算相对时间（以2018-09-01 00:00为基准）
    base_date = pd.Timestamp('2018-09-01')
    grouped['latest_hours'] = (grouped['est_arrival_date'] - base_date).dt.total_seconds() / 3600.0
    grouped['earliest_hours'] = grouped['latest_hours'] - grouped['transit_days'] * 24.0
    
    orders = []
    for _, row in grouped.iterrows():
        std_addr = dm.resolve_address(row['receiver_addr'])
        # 生成唯一的订单ID（交接单号_地址短标识）
        addr_short = ''.join([c for c in row['receiver_addr'] if c.isalnum()])[:10]
        oid = f"{row['trans_receipt_no']}_{addr_short}"
        orders.append(Order(
            order_id=oid,
            address=row['receiver_addr'],
            standard_addr=std_addr,
            pallets=row['total_pallets'],
            earliest_arrival=row['earliest_hours'],
            latest_arrival=row['latest_hours'],
            dm=dm
        ))
    
    print(f"  从{len(xls.sheet_names)}个工作表中共解析出 {len(orders)} 个配送订单")
    return orders
...
def main():
    ...
    # 1. 加载距离/时间矩阵
    print("\n[1] 加载距离和时间矩阵...")
    dm = RealDistanceMatrix("distance_matrix_fixed.xlsx", "duration_matrix_fixed.xlsx")
